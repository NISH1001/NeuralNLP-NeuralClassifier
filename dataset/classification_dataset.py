...
class ClassificationDataset(DatasetBase):
    ...
    def _init_dict(self):
        ...
        self.id_to_vocab_dict_list = [
            ...
            self.id_to_char_map,
            self.id_to_token_gram_map,
            self.id_to_keyword_map,
    ...
    def _get_vocab_id_list(self, json_obj):
        ...
        token_ids, char_ids, char_in_token_ids, token_ngram_ids = self._token_to_id(
            doc_tokens,
            self.token_map,
            self.char_map,
            self.config.feature.token_ngram,
        ...
        return {
            self.DOC_LABEL: self._label_to_id(doc_labels, self.label_map)
            if self.model_mode != ModeType.PREDICT
            else [0],
...
class BertDataset(ClassificationDataset):
    def __init__(self, config, json_files, generate_dict=False, mode=ModeType.EVAL):
        super().__init__(config, json_files, generate_dict=generate_dict, mode=mode)
