...
class ActivationType(Type):
    ...
    @classmethod
    def str(cls):
        return ",".join([cls.SIGMOID, cls.TANH, cls.RELU, cls.LEAKY_RELU, cls.NONE])
...
def get_optimizer(config, params):
    ...
    if config.optimizer.optimizer_type == OptimizerType.ADAM:
        return torch.optim.Adam(lr=config.optimizer.learning_rate, params=params)
        ...
        return BertAdam(
            params, lr=config.optimizer.learning_rate, weight_decay=0, max_grad_norm=-1
        )
    else:
        raise TypeError(
...
def get_hierar_relations(hierar_taxonomy, label_map):
    ...
    with cs.open(hierar_taxonomy, "r", "utf8") as f:
        for line in f:
            ...
            children_label_ids = [
                label_map[child_label]
                for child_label in children_label
                if child_label in label_map
            ]
            hierar_relations[parent_label_id] = children_label_ids
    return hierar_relations


def get_hierar_relations_new(hierar_taxonomy, label_map):
    """get parent-children relationships from given hierar_taxonomy
    hierar_taxonomy: parent_label \t child_label_0 \t child_label_1 \n


    Bugfix from:
        https://github.com/Tencent/NeuralNLP-NeuralClassifier/issues/86
    """
