...
class HMCN(Classifier):
    ...
    def __init__(self, dataset, config):
        ...
        for i in range(1, len(self.hierarchical_depth)):
            ...
            self.local_layers.append(
                torch.nn.Sequential(
                    ...
                    torch.nn.Linear(
                        self.global2local[i], self.hierarchical_class[i - 1]
                    ),
                )
            )
        ...
        self.linear = torch.nn.Linear(
            self.hierarchical_depth[-1], len(dataset.label_map)
        )
    ...
    def forward(self, batch):
        ...
        for i, (local_layer, global_layer) in enumerate(
            zip(self.local_layers, self.global_layers)
        ):
            ...
            if i < len(self.global_layers) - 1:
                global_layer_activation = torch.cat(
                    (local_layer_activation, doc_embedding), 1
                )
            else:
        ...
        return (
            global_layer_output,
            local_layer_output,
            0.5 * global_layer_output + 0.5 * local_layer_output,
        )
